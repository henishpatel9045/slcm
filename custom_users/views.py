...
from .serializers import (
    CentralEducationDepartmentSerializer,
    DistrictEduSerializer,
...
class StudentDetailViewSet(PaginatorSerializer):
    ...
    def get_queryset(self):
        ...
        if hasattr(user, "app_user"):
            ...
            qs = Student.objects.filter(
                state=user.district_edu.state, district=user.district_edu.district
            )
...
class UserDetails(views.APIView):
    ...
    def get(self, request):
        ...
        if hasattr(user, "app_user"):
            ...
            qs = (
                InstituteAdmin.objects.select_related("institute")
                .select_related("user")
                .get(user=user)
            )
        elif hasattr(user, "central_edu"):
            serializer = CentralEducationDepartmentSerializer
            qs = CentralEducationDepartment.objects.select_related("user").get(
                user=user
            )
        elif hasattr(user, "state_edu"):
            ...
            qs = DistrictEducationDepartment.objects.select_related("user").get(
                user=user
            )
        ...
        if serializer:
            ...
            return Response(
                {"detail": "User is not valid.", "success": False},
...
class GenerateResetOTP(views.APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"detail": "Error occurred.", "error": str(e.args[0])},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class PasswordReset(views.APIView):
    def post(self, request):
        ...
        if check_password(password, user.password):
            return Response(
                {
                    "error": ["Can't use current password as new password."],
                    "sucess": False,
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"error": e.args[0][0], "success": False},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            {
                "detail": f"PAssword changed for user {user.username}. New password is {password}",
                "success": True,
            },
...
class Logout(views.APIView):
    def post(self, request):
        try:
            ...
            return Response(
                {"error": str(e.args[0]), "status": False},
                status=status.HTTP_400_BAD_REQUEST,
            )
        return Response({"status": True}, status=status.HTTP_200_OK)
...
class StudentUpdateViewSet(viewsets.ModelViewSet):
    queryset = StudentChangeRequest.objects.all()
    serializer_class = StudentUpdateRequest
    ...
    def get_queryset(self):
        ...
        if hasattr(user, "app_user"):
            ...
            qs = qs.filter(
                student__enrollement__institute=user.institute_admin.institute,
                student__enrollement__is_active=True,
            )
        elif hasattr(user, "central_edu"):
            pass
        elif hasattr(user, "state_edu"):
            qs = qs.filter(student__state=user.state_edu.state)
        elif hasattr(user, "district_edu"):
            qs = qs.filter(
                student__district=user.district_edu.district,
                student__state=user.district_edu.state,
            )

        return qs


class BulkApproveStudentEnrollement(views.APIView):
    def post(self, request):
        try:
            data = request.data.get("students")
            students = []
            for student in data:
                students.append(Student.objects.get(id=student))

            if students:
                Student.objects.bulk_update(
                    students, is_approved=True, enrollement__is_active=True
                )

            return Response(
                {"detail": "Successfully approved new students.", "status": True},
                status=status.HTTP_200_OK,
            )
        except Exception as e:
            return Response(
                {"detail": "Error occurred.", "error": str(e.args[0]), "status": False},
                status=status.HTTP_400_BAD_REQUEST,
            )


class StudentForApproval(mixins.ListModelMixin, viewsets.GenericViewSet):
    queryset = Student.objects.filter(is_approved=False)

    def get_queryset(self):
        user = self.request.user

        qs = self.queryset
        if hasattr(user, "central_edu"):
            pass
        elif hasattr(user, "app_user"):
            qs = qs.filter(student__user=user)
        elif hasattr(user, "state_edu"):
            qs = qs.filter(
                student__state=user.state_edu.state,
            )
        elif hasattr(user, "district_edu"):
            qs = qs.filter(
                student__district=user.district_edu.district,
    ...
    def list(self, request, *args, **kwargs):
        institute = self.kwargs.get("institute")

        if institute:
            self.queryset = self.queryset.filter(
                enrollement__institute__institute_id=institute
            )
